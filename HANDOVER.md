...
### 2026-06-26 セッション（後半）— Phase 1+2 実装 ✅（**人間未検証**）

ユーザー画像で確認されたピアノロール重複を含む報告に対し、以下を追加実装（`cargo test` 4件通過）。

| # | 報告 | 対応 |
|---|------|------|
| 1 | ピアノノート大量重複 | `replace_notes_in_range` — Generate All が **範囲内置換**（`extend` 廃止） |
| 2 | シンコペ後の休符 + refill 重複 | `note_overlaps_range` で overlap 除去；`pattern_tile_at` 位相修正 |
| 3 | ピッチ2オクターブ高い | median 自動推定を廃止し specs アンカー（60/48）固定 + 回帰テスト |
| 4 | Ctrl+C/V 未実装感 | `piano_roll_focused` + `handle_edit_shortcuts`（C/X/V/D/A、トースト） |
| 5 | 複数選択不可 | ピアノロール **Shift+クリック** トグル、Shift+drag 箱選択、矢印ナッジ |
...
### 優先 0 — **ユーザー GUI 検証**（Phase 1+2 の確認）
- [ ] Generate 2回押してもノートが重複しない（画像のような層にならない）
- [ ] Generate ピッチが妥当か（Piano/Bass、2オクターブ高くない）
- [ ] シンコペ ★ 後、長い休符が残らない
- [ ] Shift+クリック複数選択 → Ctrl+C/V/X → 再生ヘッドへペースト
- [ ] 矢印キーでナッジ（Shift=Len 単位）
- [ ] 伸縮 / コード重なり / patch 変更（前スプリント分も再確認）
