...
| 文書 | 役割 |
|------|------|
| **[`SPEC-v2.md`](SPEC-v2.md)** | **仕様の真実**（五本柱・契約・Reshape 方針） |
| [`SPEC-v1.md`](SPEC-v1.md) | 履歴（v2 が優先） |
| [`HANDOVER.md`](HANDOVER.md) | 実装ギャップ・次の一手 |
| [`ENV.md`](ENV.md) | WSL2 + Grok Build |
| [`tests/golden/ACCEPTANCE.md`](tests/golden/ACCEPTANCE.md) | 手動受け入れ（v2 向け更新予定） |
...
**現行の完成定義・作業順は [`SPEC-v2.md`](SPEC-v2.md) のみ。**  
ギャップは [`HANDOVER.md`](HANDOVER.md)。手動ゲートは [`tests/golden/ACCEPTANCE.md`](tests/golden/ACCEPTANCE.md)。

v1 文書・旧 Phase チェックリストと矛盾する場合は **SPEC-v2 が勝つ**。方針は **Reshape（捨てずに美しく付け替える）**。

**意図的に後回し (v2.1+):** 無限タイムライン、オーディオ録音、フルミキサー、Chrome 拡張必須化、Domino 全機能。
