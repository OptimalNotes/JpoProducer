...
### 見る
- **オニオン**: キースケール（薄ピンク）+ コードトーン（薄青、ブロック範囲）、**Onion** 濃さスライダー
- トラック名短縮: `Chord` / `Ch2` … / `Drum`、音色名は固定幅で省略表示

### その他
- Key / BPM / Major-Minor（**現状は曲全体1設定 → 将来ループ単位に移行予定**）
- ジェネレータ（Piano Ch2 / Bass Ch3 / Drum Ch10）、Generate range（デフォルト 0–16拍 = 4小節）
...
### Phase C — 曲にする
- [ ] **Arrange モード**（ループ列の並べ替え・重複・通しプレビュー）
- [ ] Arrange から **フル尺 MIDI Export**
- [ ] **転調補助ツール** = コード進行テンプレ（Ch1 へ一発配置）

### Phase D — 余裕があれば
- [ ] MIDI Import（1ループ分）
- [ ] ジェネレータパターン増（8ビート / シンコペ等）
- [ ] 日本語フォント同梱
